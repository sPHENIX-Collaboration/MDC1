...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'DST_CALO_CLUSTER' and (events is null or events < 0) and filename like '%0_488fm%' order by filename") || die $DBI::error;
