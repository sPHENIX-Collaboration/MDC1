...
my $getfiles = $dbh->prepare("select filename from datasets,files where datasets.dsttype = 'DST_CALO_CLUSTER' and datasets.filename = files.lfn and files.full_host_name = 'gpfs' and files.md5 is null and datasets.filename like '%0_488fm%' order by filename");
