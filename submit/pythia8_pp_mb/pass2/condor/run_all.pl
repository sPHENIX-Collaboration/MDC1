...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and filename like '%pythia8_mb%' order by filename") || die $DBI::error;
...
# output file does not exist yet, check for 2 MB background files (n to n+1)
	...
	for (my $cnt = $segment+1; $cnt <=$segment+100; $cnt++)
	{
