...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and filename like 'G4Hits_pythia8_mb%' and (events is null or events < 0) order by filename") || die $DBI::error;
