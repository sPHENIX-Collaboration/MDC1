...
my @dcpath = ();
push(@dcpath,"/pnfs/rcf.bnl.gov/sphenix/disk/MDC1/pythia8_pp/G4Hits");
push(@dcpath,"/pnfs/rcf.bnl.gov/phenix/sphenixraw/MDC1/pythia8_pp/G4Hits");
...
my $getmyfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'G4Hits_pythia8_mb%' and full_file_path like '$dcpath[$dcache]%' order by lfn");
my $getgpfsfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'G4Hits_pythia8_mb%' and full_host_name = 'gpfs' order by lfn");
my $getotherfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'G4Hits_pythia8_mb%' and full_file_path like '$dcpath[$otherdcache]%' order by lfn");
my %gpfsfiles = ();
my %myfiles = ();
my %otherfiles = ();
...
#    print F2 "  system(\"xrdcp $infile root://dcsphdoor02.rcf.bnl.gov:1095$dcfile\");\n";
