...
my $getfiles = $dbh->prepare("select filename from datasets,files where datasets.dsttype = 'G4Hits' and datasets.filename = files.lfn and full_host_name = 'gpfs' and files.md5 is null and filename like 'G4Hits_sHijing_0_12fm%' order by filename");
