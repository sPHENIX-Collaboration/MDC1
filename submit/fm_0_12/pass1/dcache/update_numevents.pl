...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and (events is null or events < 0) and filename like 'G4Hits_sHijing_0_12fm%' order by filename") || die $DBI::error;
