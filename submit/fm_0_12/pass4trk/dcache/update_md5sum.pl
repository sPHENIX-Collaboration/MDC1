...
my $getfiles = $dbh->prepare("select filename from datasets,files where datasets.dsttype = 'DST_TRACKS' and datasets.filename = files.lfn and files.full_host_name = 'gpfs' and files.md5 is null and filename like 'DST_TRACKS_sHijing_0_12fm%' order by filename");
