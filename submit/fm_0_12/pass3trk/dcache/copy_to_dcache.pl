...
my $getmyfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_TRKR_CLUSTER_sHijing_0_12fm%' and full_file_path like '$dcpath[$dcache]%' order by lfn");
my $getgpfsfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_TRKR_CLUSTER_sHijing_0_12fm%' and full_host_name = 'gpfs' order by lfn");
my $getotherfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_TRKR_CLUSTER_sHijing_0_12fm%' and full_file_path like '$dcpath[$otherdcache]%' order by lfn");
my %gpfsfiles = ();
my %myfiles = ();
my %otherfiles = ();
