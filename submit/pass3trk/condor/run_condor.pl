...
if ($#ARGV < 3)
{
    print "usage: run_condor.pl <events> <trk infile> <truth infile> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/pass2");
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile0 $infile1 $outfile $dstoutdir\"\n";
