...
then
    ...
    rsync -av /sphenix/u/sphnxpro/MDC1/submit/fm_0_20/pass3trk_test/rundir/* .
    getinputfiles.pl $2
    if [ $? -ne 0 ]
    then
...
echo running root.exe -q -b Fun4All_G4_TrkrPrePass.C\($1,\"$2\",\"$3\",\"$4\",\"$5\"\)
root.exe -q -b  Fun4All_G4_TrkrPrePass.C\($1,\"$2\",\"$3\",\"$4\",\"$5\"\)
echo "script done"
