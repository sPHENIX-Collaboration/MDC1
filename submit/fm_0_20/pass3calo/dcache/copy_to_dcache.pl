...
my @dcpath = ();
push(@dcpath,"/pnfs/rcf.bnl.gov/sphenix/disk/MDC1/sHijing_HepMC/CaloCluster");
push(@dcpath,"/pnfs/rcf.bnl.gov/phenix/sphenixraw/MDC1/sHijing_HepMC/CaloCluster");
...
my $getmyfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_CALO_CLUSTER_sHijing_0_20fm_50kHz_bkg_0_20fm%' and full_file_path like '$dcpath[$dcache]%' order by lfn");
my $getgpfsfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_CALO_CLUSTER_sHijing_0_20fm_50kHz_bkg_0_20fm%' and full_host_name = 'gpfs' order by lfn");
my $getotherfiles = $dbh->prepare("select lfn,full_file_path from files where lfn like 'DST_CALO_CLUSTER_sHijing_0_20fm_50kHz_bkg_0_20fm%' and full_file_path like '$dcpath[$otherdcache]%' order by lfn");
my %gpfsfiles = ();
my %myfiles = ();
my %otherfiles = ();
