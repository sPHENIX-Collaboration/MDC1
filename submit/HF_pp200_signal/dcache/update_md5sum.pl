...
my $getfiles = $dbh->prepare("select filename from datasets,files where datasets.dsttype = 'G4sPHENIX' and datasets.filename = files.lfn and files.full_host_name = 'gpfs' and files.md5 is null order by filename");
